...
if __name__ == '__main__':
    ...
    concat_data_full = pd.concat(raw_data, axis=1)
    
    concat_data = concat_data_full.head(10000)
...
def input_fn(input_data, content_type):
    ...
    if content_type == "text/csv":
        ...
        if len(df.columns) == len(params.feature_dtypes_all.items()) + 1:
            # This is a labelled example, includes the ring label
            df.columns = params.feature_dtypes_all.items() + [params.label_column]
        elif len(df.columns) == len(params.feature_dtypes_all.items()):
            # This is an unlabelled example.
            df.columns = params.feature_dtypes_all.items()
